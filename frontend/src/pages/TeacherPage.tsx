  ...
  const startRecording = () => {
    if (streamRef.current) {
      ...
      if (!videoTrack || !audioTrack) {
        setError('Cannot record: Missing video or audio track');
        return;
      }
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const videoUrl = `${import.meta.env.VITE_API_URL}/videos/${data.filename}`;
            await fetch(`${import.meta.env.VITE_API_URL}/api/courses/${courseId}/lessons/${lessonId}`, {
              method: 'PUT',
              headers: {
            ...
            alert(`Video uploaded! You can view it at /videos/${data.filename}`);
          } else {
